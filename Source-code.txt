...
static Grid BuildMiniPlayerFlyoutContent() {
    ...
    // Original fixed neutral acrylic — same as the very first version of
    // this popup, independent of the player's own BackgroundStyleSettings.
    Brush flyoutContentBackground{nullptr};

if (isLight) {
    flyoutContentBackground = MakeBrush(
        winrt::Windows::UI::Color{0xFF, 0xF2, 0xF2, 0xF2}
    );
} else {
    flyoutContentBackground = MakeBrush(
        winrt::Windows::UI::Color{0xFF, 0x15, 0x15, 0x15}
    );
}

bg.Background(flyoutContentBackground);
    ...
    // ---- Header: small album art + title/artist + optional settings icon ----
    Grid headerGrid;
    headerGrid.HorizontalAlignment(HorizontalAlignment::Stretch);
    headerGrid.Margin({0, 0, 0, 10});
    ColumnDefinition hcArt, hcText, hcMenu;
    hcArt.Width({1.0, GridUnitType::Auto});
    hcText.Width({1.0, GridUnitType::Star});
    hcMenu.Width({1.0, GridUnitType::Auto});
    headerGrid.ColumnDefinitions().Append(hcArt);
    headerGrid.ColumnDefinitions().Append(hcText);
    headerGrid.ColumnDefinitions().Append(hcMenu);
 
    Border artBorder;
    artBorder.Width(56);
    artBorder.Height(56);
    artBorder.VerticalAlignment(VerticalAlignment::Center);
    ...
    headerGrid.Children().Append(artBorder);
    ...
    textStack.Children().Append(wrapDebugBorder(titleBlock, {0xFF, 0xFF, 0xFF, 0x00}));
    ...
    artistBlock.HorizontalAlignment(HorizontalAlignment::Left);
    artistBlock.TextAlignment(TextAlignment::Left);
    artistBlock.Opacity(0.65);
    artistBlock.Foreground(MakeBrush(ArtistColor()));
    artistBlock.Margin({0, 2, 0, 0});
    artistBlock.Text(artist.empty() ? winrt::hstring(L" ") : winrt::hstring(artist));
    textStack.Children().Append(wrapDebugBorder(artistBlock, {0xFF, 0x00, 0xFF, 0x00}));
    ...
    if (g_settings.showOpenWindhawk) {
        ...
        menuBtn.CornerRadius({6, 6, 6, 6});
        menuBtn.BorderThickness({0, 0, 0, 0});
        ...
        menuIcon.FontFamily(Media::FontFamily(L"Segoe Fluent Icons"));
        menuIcon.Foreground(MakeBrush(isLight
        ...
        menuBtn.Content(winrt::box_value(menuIcon));
 
        ToolTip menuToolTip;
        menuToolTip.Content(winrt::box_value(winrt::hstring(L"Open Windhawk")));
        ToolTipService::SetToolTip(menuBtn, menuToolTip);
 
        menuBtn.Click([](auto const&, auto const&) { OpenWindhawk(); });
 
        ApplyFluentMediaButtonStyle(menuBtn);
        auto menuHovered = std::make_shared<bool>(false);
        auto menuApplyState = [menuBtn, menuHovered](bool pressed) {
            try {
                GoToCommonState(menuBtn,
                    IsHoverEffectEnabled(g_settings.mediaButtonsHoverEffectMode),
                    pressed, *menuHovered);
            } catch (...) {}
        };
        RunWhenButtonReady(menuBtn, [menuBtn, menuApplyState]() {
            try {
                SetupMediaButtonCommonStates(menuBtn);
                menuApplyState(false);
            } catch (...) {}
        });
        menuBtn.PointerEntered([menuHovered, menuApplyState](auto const&, auto const&) {
            *menuHovered = true; menuApplyState(false);
        });
        menuBtn.PointerExited([menuHovered, menuApplyState](auto const&, auto const&) {
            *menuHovered = false; menuApplyState(false);
        });
        menuBtn.AddHandler(UIElement::PointerPressedEvent(), winrt::box_value(
            winrt::Windows::UI::Xaml::Input::PointerEventHandler(
            [menuApplyState](auto const& sender, winrt::Windows::UI::Xaml::Input::PointerRoutedEventArgs const& e) {
            try {
                if (auto el = sender.template try_as<UIElement>()) el.CapturePointer(e.Pointer());
                menuApplyState(true);
            } catch (...) {}
        })), true);
        menuBtn.AddHandler(UIElement::PointerReleasedEvent(), winrt::box_value(
            winrt::Windows::UI::Xaml::Input::PointerEventHandler(
            [menuApplyState](auto const& sender, winrt::Windows::UI::Xaml::Input::PointerRoutedEventArgs const& e) {
            try {
                if (auto el = sender.template try_as<UIElement>()) el.ReleasePointerCapture(e.Pointer());
                menuApplyState(false);
            } catch (...) {}
        })), true);
 
        Grid::SetColumn(menuBtn, 2);
        headerGrid.Children().Append(menuBtn);
    }
 
    content.Children().Append(wrapDebugBorder(headerGrid, {0xFF, 0x00, 0x80, 0xFF}));
    ...
    controlsRow.Margin({0, 0, 0, 4});
 
    auto iconClr = ButtonColor();
 
    auto MakeBigBtn = [&](int cmd, bool playing, bool executeCommand = true, double sizePx = 34.0) -> Button {
        Button btn;
        btn.Width(sizePx);
        btn.Height(sizePx);
        btn.Padding({1, 1, 1, 1});
        btn.CornerRadius({sizePx / 2.0, sizePx / 2.0, sizePx / 2.0, sizePx / 2.0});
        btn.BorderThickness({0, 0, 0, 0});
        btn.VerticalAlignment(VerticalAlignment::Center);
        btn.HorizontalAlignment(HorizontalAlignment::Center);
        ...
        // Play gets a subtle tinted capsule behind it so it still reads
        // as the primary action at the smaller compact scale.
        if (cmd == 2) {
            btn.Background(MakeBrush({0x18, iconClr.R, iconClr.G, iconClr.B}));
        }
 
        TextBlock iconText;
        iconText.Text(winrt::hstring(glyph));
        iconText.FontSize(cmd == 2 ? 15 : 13);
    ...
    Button playBtn    = MakeBigBtn(2, isPlaying, true, 40.0);
    ...
    // ---- Optional session-switch list (built/filled in by RefreshMiniPlayerFlyoutUI) ----
    {
        Border sep;
        sep.Height(1);
        sep.Margin({-12, 6, -12, 0});
    ...
    if (g_settings.showMiniPlayerBorder) {
        content.BorderBrush(MakeBrush({0xFF, 0x80, 0x80, 0xFF}));
        ...
        try {
            ...
            if (xamlRoot) {
                ...
                if (rootContent) {
                    ...
                    if (!anchorComputed) {
                        // Anchor to the player widget's own LEFT edge — a fixed
                        // on-screen coordinate, since the player wrapper is
                        // HorizontalAlignment::Left inside its column. This is
                        // deliberately NOT based on the player's width, so the
                        // popup's horizontal position is unaffected by whether
                        // a track is playing, paused, or how long the title
                        // text is.
                        //
                        // miniPlayerHorizontalOffsetNear is then applied as-is,
                        // with NO clamping — it's a setting you control
                        // explicitly to nudge the popup left (negative) or
                        // right (positive) of that edge, so it's always
                        // honored exactly as entered.
                        auto xform = target.TransformToVisual(rootContent);
                        auto pt = xform.TransformPoint({0.f, 0.f});
                        float leftX = pt.X + (float)g_settings.miniPlayerHorizontalOffsetAbove;
 
                        bool placeBelow = (g_settings.miniPlayerVerticalPlacementNear == L"bottom");
                        if (placeBelow) {
                            float ty = pt.Y + (float)target.ActualHeight();
                            anchorPoint = {leftX, ty};
                            placementMode = Controls::Primitives::FlyoutPlacementMode::BottomEdgeAlignedLeft;
                            g_miniPlayerAnimAxis = 0;
                            g_miniPlayerAnimSign = -1.0;
                        } else {
                            float ty = pt.Y;
                            anchorPoint = {leftX, ty};
                            placementMode = Controls::Primitives::FlyoutPlacementMode::TopEdgeAlignedLeft;
                            g_miniPlayerAnimAxis = 0;
                            g_miniPlayerAnimSign = 1.0;
                        }
                    }
